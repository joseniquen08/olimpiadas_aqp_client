...
export function AssignDelegateModal({ event_id, delegates, delegatesTotal }: Props) {
  ...
  const onSubmit = async (data: z.infer<typeof FormSchema>) => {
    ...
    const response = await fetch(`/api/event/assign/delegate`, {
      method: 'POST',
      body: JSON.stringify({
        eventId: event_id,
            ...
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              ...
              >
                <Dialog.Panel className="w-full max-w-3xl transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  <Dialog.Title as="h3" className="text-2xl font-bold leading-6 text-emerald-900">Asignar delegado al evento</Dialog.Title>
                                ...
                                <Command>
                                  ...
                                  <CommandGroup>
                                    {delegatesTotal.map((delegate: any) => {
                                      return delegates.delegates.find((dg: any) => dg.delegateId == delegate.delegateId) != undefined ? (
                                        <CommandItem
