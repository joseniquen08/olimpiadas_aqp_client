...
const FormSchema = z.object({
  ...
  nickname: z.string({
    required_error: "El apodo es obligatorio"
  }).min(1, { message: "La apodo es obligatorio" }),
  image: z
    .custom<FileList>(val => val instanceof FileList, "La imagen es obligatoria")
    .refine(files => files.length > 0, "La imagen es obligatoria")
    .refine(files =>
      Array.from(files).every(file => file.size <= 3*1024*1024),
      "El tamaño máximo de imagen es de 3MB."
    )
    .refine(files =>
...
export function AddTeamModal({ category_id }: Props) {
  ...
  const onSubmit = async (data: z.infer<typeof FormSchema>) => {
    ...
    const dataResImage = await responseImage.json();
            ...
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              ...
              >
                <Dialog.Panel className="w-full max-w-3xl transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  <Dialog.Title as="h3" className="text-2xl font-bold leading-6 text-emerald-900">Agregar un nuevo equipo</Dialog.Title>
                  ...
                  <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="pt-6 grid grid-cols-2 gap-4 md:gap-6">
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem className="col-span-1 flex flex-col space-y-2.5">
                            <FormLabel>Nombre del equipo <span className="text-red-500">*</span></FormLabel>
                            <FormControl>
                              <Input placeholder="Equipo" {...field} autoComplete="off" />
                            </FormControl>
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem className="col-span-1 flex flex-col space-y-2.5">
                            <FormLabel>Apodo <span className="text-red-500">*</span></FormLabel>
                            <FormControl>
                              <Input placeholder="Apodo" {...field} autoComplete="off" />
                            </FormControl>
                            <FormMessage className="text-xs text-red-400" />
                          </FormItem>
                        )}
